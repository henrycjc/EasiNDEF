...
NdefMessage::NdefMessage(const NdefMessage& rhs) {
    ...
    for (uint32_t i = 0; i < _recordCount; i++) {
...
uint32_t NdefMessage::getRecordCount() {
    return _recordCount;
}

uint32_t NdefMessage::getEncodedSize() {
    uint32_t size = 0;
    for (uint32_t i = 0; i < _recordCount; i++) {
...
NdefRecord NdefMessage::getRecord(uint32_t index) {
